...
insert_data = '''INSERT INTO codigos_postales (d_codigo, d_asenta, d_tipo_asenta, 
                                            D_mnpio, d_estado, d_ciudad, 
                                            d_CP, c_estado, c_oficina,
                                            c_CP, c_tipo_asenta, c_mnpio, 
                                            id_asenta_cpcons, d_zona, c_cve_ciudad) 
                VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s,%s)'''

# Insert DataFrame to Table
for row in df.itertuples():
    cursor.execute(insert_data, (row.d_codigo, row.d_asenta, row.d_tipo_asenta, 
                                row.D_mnpio,row.d_estado, row.d_ciudad, 
                                row.d_CP, row.c_estado, row.c_oficina,
